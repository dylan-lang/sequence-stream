...
define test position-offset-test ()
  ...
  check-equal("start position check",
              30,
              stream.stream-position);
  check-equal("size check",
              text.size + 30,
              stream.stream-size);
  stream.stream-position := #"end";
  check-equal("end position check",
              text.size + 30,
              stream.stream-position);
end test;
...
define test read-test ()
  ...
  check-equal("expected data check",
              "Hello.\nMy",
              read(stream, 9));
  check-condition("EOS condition check",
                  <incomplete-read-error>,
                  read(stream, 100));
  check-equal("EOS value check",
              #t,
...
define test read-line-test ()
  ...
  check-condition("EOS condition check", <end-of-stream-error>, stream.read-line);
end test;
